...
[ -f "$TAILSCALE_CONFIG" ] || { echo "missing canonical tailscale config"; exit 1; }

grep -Fq "option accept_routes '1'" "$TAILSCALE_CONFIG" || {
	echo "canonical Tailscale route-acceptance default must remain enabled"
	exit 1
}

if grep -Eq 'HEADSCALE_OPENWRT_ACCEPT_ROUTES|set_config_option accept_routes' "$ENROLL_SCRIPT"; then
	echo "build helper must not inject a duplicate route-acceptance policy"
	exit 1
fi
grep -Fq "set_config_option advertise_routes ''" "$ENROLL_SCRIPT"
grep -Fq 'tailnet_accept_routes()' "$ENROLL_BIN"
grep -Fq 'tailscale.settings.accept_routes' "$ENROLL_BIN"
grep -Fq 'accept_routes="$(tailnet_accept_routes)"' "$ENROLL_BIN"
