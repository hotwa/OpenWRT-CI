...
allowed_secrets='HEADSCALE_OPENWRT_AUTHKEY HEADSCALE_CI_AUTHKEY HEADSCALE_URL MULTICA_TOKEN MULTICA_SERVER_URL MULTICA_APP_URL MULTICA_WORKSPACE_ID OPENWRT_DROPBEAR_AUTHORIZED_KEYS OPENWRT_WAN_PPPOE_USERNAME OPENWRT_WAN_PPPOE_PASSWORD NIKKI_SUBSCRIPTION_URL COMMANDCODE_API_KEY CLIPROXYAPI_API_KEY CLIPROXYAPI_BASE_URL SAMBA_DEFAULT_PASSWORD CPE5G_WIFI_PASSWORD'
...
grep -Fq 'github.repository_visibility' "$CORE" && \
  grep -Fq 'Wi-Fi candidate builds require a private repository' "$CORE" || {
  echo "CPE Wi-Fi credentials must not be embedded in artifacts from a public repository" >&2
  exit 1
}
