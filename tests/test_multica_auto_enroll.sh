...
AGENT_MD="$ROOT_DIR/files/root/.multica/openwrt-agent.md"
...
[ -f "$AGENT_MD" ] || { echo "missing openwrt-agent.md in root/.multica"; exit 1; }
...
grep -Fq 'Qualcomm IPQ6000' "$AGENT_MD"
grep -Fq 'Headscale' "$AGENT_MD"
grep -Fq 'rclone' "$AGENT_MD"
