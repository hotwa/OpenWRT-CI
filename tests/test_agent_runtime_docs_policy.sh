...
for term in 'Pi / CommandCode' '已签名的不可变 generation' 'Node.js' 'CPython 3.13' '固件不预置任何用户' '/data/node'; do
...
for retired in 'OpenCode' 'Hermes' 'build_hermes_core.sh' 'Python 3.12 (`uv`)'; do
