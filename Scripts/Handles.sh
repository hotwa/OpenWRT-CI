...
GNULIB_MAKEFILE="$PKG_PATH/../tools/gnulib/Makefile"
GNULIB_PATCH_DIR="$PKG_PATH/../tools/gnulib/patches"
GNULIB_OLD_DATE="2025-07-01"
GNULIB_NEW_DATE="2026-07-04"
...
GNULIB_FTS_SHA="2a5915114fef178ef4b5d5ea9705c3447ca4a3dbfcd476162334b41d998ee6ad"
GNULIB_OBSOLETE_PATCHES="
...
if [ -f "$GNULIB_MAKEFILE" ]; then
	if grep -q "^PKG_SOURCE_VERSION:=$GNULIB_NEW_VER" "$GNULIB_MAKEFILE"; then
		...
		sed -i "s/^PKG_SOURCE_DATE:=$GNULIB_OLD_DATE\$/PKG_SOURCE_DATE:=$GNULIB_NEW_DATE/" "$GNULIB_MAKEFILE"
		...
		for p in $GNULIB_OBSOLETE_PATCHES; do
			rm -f "$GNULIB_PATCH_DIR/$p"
		done
		retry_cmd 5 15 curl -fsSL \
			"https://raw.githubusercontent.com/immortalwrt/immortalwrt/837f5eaae2/tools/gnulib/patches/120-unmangle-darwin-fts-h.patch" \
			-o "$GNULIB_PATCH_DIR/120-unmangle-darwin-fts-h.patch" || {
			echo "ERROR: failed to download gnulib 120-unmangle-darwin-fts-h.patch" >&2
			exit 1
		}
		grep -q "^PKG_SOURCE_DATE:=$GNULIB_NEW_DATE\$" "$GNULIB_MAKEFILE" && \
		...
		[ "$(sha256sum "$GNULIB_PATCH_DIR/120-unmangle-darwin-fts-h.patch" | cut -d' ' -f1)" = "$GNULIB_FTS_SHA" ] || {
