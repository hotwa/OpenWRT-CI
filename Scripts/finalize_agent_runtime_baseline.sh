...
ROOT_DIR="$ROOT_DIR" TARGET_FILES="$TARGET_FILES" BASELINE="$BASELINE" \
  NODE="$NODE" NODE_VERSION_FILE="$NODE_VERSION_FILE" MULTICA="$MULTICA" MANIFEST="$MANIFEST" \
...
if (!nodeVersion || !multica || !['arm64', 'x64'].includes(e.ARCH) || !Number.isSafeInteger(Number(e.NODE_ABI))) throw new Error('invalid baseline component metadata');
const manifest = {
  schema_version: 1,
  ...
  components: {...deps, multica, 'pi-plan-mode': pi.version},
