...
- **动态路由自愈**：`tailscale-route-reconcile` 根据在线 Peer 的 `PrimaryRoutes` 核对 policy-routing table 52，先做无损 netmap 刷新，连续确认失败且经过冷却后才重启 Tailscale；不会写死网段或手工添加路由。详见 `docs/tailscale-route-reconcile.md`。
