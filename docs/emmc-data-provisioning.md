...
已刷入早期固件的三种 JDCloud 设备可能已经有 `mmcblk0p27`，GPT 名称为
`data`，文件系统却没有 `openwrt-data` 标签。这是已知兼容布局，不代表损坏：

- 当 p27 是位于磁盘末端的精确 `data` 分区，且已识别为 ext4 或 f2fs，固件绝不
  格式化它。`99-auto-mount-data` 只会卸载其已知的匿名
  `/mnt/mmcblk0p27` 挂载，再以文件系统 UUID 挂到 `/data` 并写入 fstab；后续
  `sysupgrade` 会直接恢复该挂载。
- 只有同一精确 p27 未挂载且 `blkid` 完全识别不到文件系统时，才会初始化为
